...
public class WebViewPlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if (action.equals("show") && args.length() > 0) {
            ...
            if (args.getJSONArray(1) != null) {
                for (int i = 0; i < args.getJSONArray(1).length(); i++) {
                    ignoreArray.add(args.getJSONArray(1).getString(i));
                }
            }
            ...
            if (!"".equals(url)) {
                showWebView(url, ignoreArray, shouldShowLoading);
    ...
    private void showWebView(final String url, ArrayList<String> ignoreArray, Boolean shouldShowLoading) {
        LOG.d(LOG_TAG, "Url: " + url);
        Intent i = new Intent(this.cordova.getActivity(), WebViewActivity.class);
        i.putExtra("url", url);
        i.putExtra("shouldShowLoading", shouldShowLoading);
        i.putExtra("ignore", ignoreArray);
